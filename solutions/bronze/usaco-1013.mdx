...
```py
def reverse_segment(cows, start, end):
    while start < end:
        cows[start], cows[end] = cows[end], cows[start]
        start += 1
        end -= 1

def find_cycle_length(cows, a1, a2, b1, b2):
    # simulate the swap operation until we see a repeat
    visited = set()
...
def apply_swaps(cows, a1, a2, b1, b2, k):
    cycle_len = find_cycle_length(cows, a1, a2, b1, b2)
    k %= cycle_len
    for i in range(k):
        reverse_segment(cows, a1 - 1, a2 - 1)
        reverse_segment(cows, b1 - 1, b2 - 1)
...
cows = apply_swaps(cows, a1, a2, b1, b2, k)
